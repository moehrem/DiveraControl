...
async def async_setup_entry(
    ...
    def _async_update_calendar() -> None:
        ...
        event_items: dict[str, Any] = coordinator.data.get(D_EVENTS, {}).get(
            "items", {}
        )

        # if there is no calendar entry, we'll receive an empty list from Divera - but we need a dict
        if not isinstance(event_items, dict):
            _LOGGER.debug(
                "Event items is %s (expected dict), clearing calendar",
                type(event_items).__name__,
            )
            event_items = {}
